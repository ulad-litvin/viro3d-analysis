...
cf_json_file = "./all_rank_001_CF_plddt_scores.json"
with open(cf_json_file, 'r') as f:
    cf_plddt = json.load(f)

n = 0
print("index\trecord_id\tef_plddt_mean\tef_plddt_0_50\tef_plddt_50_70\tef_plddt_70_90\tef_plddt_90_100")
for record_id, plddt in cf_plddt.items():
    n += 1
    plddt_cov = plddt_coverage_across_default_categories(plddt)
    plddt_mean_value = plddt_mean(plddt)
    df_meta.loc[df_meta['header'] == record_id, 'cf_pLDDT_mean'] = plddt_mean_value
    df_meta.loc[df_meta['header'] == record_id, 'cf_pLDDT_0_50'] = plddt_cov[0]
    df_meta.loc[df_meta['header'] == record_id, 'cf_pLDDT_50_70'] = plddt_cov[1]
    df_meta.loc[df_meta['header'] == record_id, 'cf_pLDDT_70_90'] = plddt_cov[2]
    df_meta.loc[df_meta['header'] == record_id, 'cf_pLDDT_90_100'] = plddt_cov[3]
    print(n, record_id, plddt_mean_value, plddt_cov[0], plddt_cov[1], plddt_cov[2], plddt_cov[3], sep='\t')

df_meta.to_csv("../virus_proteomes/virus_records_cf_plddt.csv", index=False)